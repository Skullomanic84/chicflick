...
//get a list of movie genre linked to the movie id
...
//get a list of the actors linked to the movie id
...
//get a list of the movie directors linked to the specific movie id
const getDirectors = function(crewList){
    const directorList = crewList.filter(({ job }) => job === "Director"); 
    
    const directors = [];
    for (const { name } of directorList) directors.push(name);

    return directors.join(",");
}

//returns only movie trailers and teasers as an array
...
fetchDataFromServer(
  ...
  function (movie) {
    ...
    document.title = `${title} - Chicflick`;
